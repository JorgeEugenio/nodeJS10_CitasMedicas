...
export default {
	...
	methods: {
		getUsuarios(){
			fetch('http://localhost:8000/v1/usuario')
			.then(res => res.json())
			.then(data => {
				this.usuarios = data
		...
		saveUsuario() {//-------------------------------------------------------------
            ...
            if (validacion) {
                if(!this.nuevo === true) {
                    ...
                    fetch('http://localhost:8000/v1/usuario/' + this.usuario._id, {
                    method: 'PUT',
                    body: JSON.stringify(this.usuario),
                    headers: {
                        'Accept': 'application/json',
		...
		getUsuario(usuario) {
			this.nuevo = true
			fetch('http://localhost:8000/v1/usuario/' + usuario._id)
				.then(res => res.json())
				.then(data => {
		...
		deleteselectedUsuarios() {
			for (let step = 0; step < this.selectedUsuarios.length; step++) {
				fetch('http://localhost:8000/v1/usuario/' + this.selectedUsuarios[step]._id, {
					method: 'DELETE',
					headers: {
					'Accept': 'application/json',
					'Content-Type': 'application/json'
