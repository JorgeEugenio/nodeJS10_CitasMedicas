...
export default {
	...
	methods: {
		getPacientes(){
			fetch('http://localhost:8000/v1/paciente')
			.then(res => res.json())
			.then(data => {
				this.pacientes = data
			})
		...
		savePaciente() {//-------------------------------------------------------------
            ...
            if (validacion) {
                if(!this.nuevo === true) {
                    ...
                    fetch('http://localhost:8000/v1/paciente/' + this.paciente._id, {
                    method: 'PUT',
                    body: JSON.stringify(this.paciente),
                    headers: {
                        'Accept': 'application/json',
		...
		getPaciente(paciente) {
			this.nuevo = true
			fetch('http://localhost:8000/v1/paciente/' + paciente._id)
				.then(res => res.json())
				.then(data => {
		...
		deleteselectedPacientes() {
			for (let step = 0; step < this.selectedPacientes.length; step++) {
				fetch('http://localhost:8000/v1/paciente/' + this.selectedPacientes[step]._id, {
					method: 'DELETE',
					headers: {
					'Accept': 'application/json',
					'Content-Type': 'application/json'
