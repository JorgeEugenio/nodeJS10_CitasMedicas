...
export default {
	...
	methods: {
		getEspecialidades(){
			fetch('http://localhost:8002/v1/especialidad')
			.then(res => res.json())
			.then(data => {
		...
		saveEspecialidad() {//-------------------------------------------------------------
            ...
            if (validacion) {
                if(!this.nuevo === true) {
                    ...
                    fetch('http://localhost:8002/v1/especialidad/' + this.especialidad._id, {
                    method: 'PUT',
                    body: JSON.stringify(this.especialidad),
                    headers: {
                        'Accept': 'application/json',
		...
		getEspecialidad(especialidad) {
			this.nuevo = true
			fetch('http://localhost:8002/v1/especialidad/' + especialidad._id)
				.then(res => res.json())
				.then(data => {
		...
		deleteselectedEspecialidads() {
			for (let step = 0; step < this.selectedEspecialidads.length; step++) {
				fetch('http://localhost:8002/v1/especialidad/' + this.selectedEspecialidads[step]._id, {
					method: 'DELETE',
					headers: {
					'Accept': 'application/json',
					'Content-Type': 'application/json'
