...
export default {
	...
	methods: {
		...
		async getDoctores(){
			await axios.get('http://localhost:8000/v1/doctoresdetalles')
			.then(data => {
				this.doctores = data.data
			})
		},
		async getEspecialidades(){
			await axios.get('http://localhost:8000/v1/especialidad')
			.then(data => {
		...
		saveDoctor() {//-------------------------------------------------------------
			...
			if (validacion) {
                if(!this.nuevo === true) {
                    fetch('http://localhost:8000/v1/doctor', {
                    method: 'POST',
                    ...
                    fetch('http://localhost:8000/v1/doctor/' + this.doctor._id, {
                    method: 'PUT',
                    body: JSON.stringify(this.doctor),
                    headers: {
                        'Accept': 'application/json',
		...
		deleteSelectedDoctors() {
			for (let step = 0; step < this.selectedDoctors.length; step++) {
				fetch('http://localhost:8000/v1/doctor/' + this.selectedDoctors[step]._id, {
					method: 'DELETE',
					headers: {
					'Accept': 'application/json',
					'Content-Type': 'application/json'
